# Plan: Daily / Weekly / Monthly Streaks

## Overview

Add three streak modes to the home screen — daily, weekly, and monthly — each backed by its own puzzle pack. Tapping a streak card navigates straight into the puzzle. The win banner shows the player's current streak. All streak state is persisted to MMKV alongside existing progress.
...
## Step 2 — Streak Date Utilities

### `src/utils/streakDate.ts`

Pure functions for computing which puzzle index to show and whether a streak is still active. All date functions use device-local time (`new Date()`), so puzzles rotate based on the player's local timezone — your "day" is your local day.
...
Add streak pack imports alongside the existing packs.
...
const DEFAULT_STREAK: Streak = { current: 0, lastCompletedKey: '' };

const DEFAULT_STREAKS: Record<StreakType, Streak> = {
  daily: { ...DEFAULT_STREAK },
  weekly: { ...DEFAULT_STREAK },
  monthly: { ...DEFAULT_STREAK },
};

export function getStreaks(): Record<StreakType, Streak> {
  const json = storage.getString(STREAKS_KEY);
  if (!json) return DEFAULT_STREAKS;
  return { ...DEFAULT_STREAKS, ...JSON.parse(json) };
}

export function saveStreak(type: StreakType, streak: Streak): void {
  const current = getStreaks();
  storage.set(STREAKS_KEY, JSON.stringify({ ...current, [type]: streak }));
}
```

The puzzle ID for streak puzzles follows the pattern `"daily:2026-02-21"`, `"weekly:2026-W08"`, `"monthly:2026-02"`. Progress is stored via the existing `getProgress`/`saveProgress` using these IDs.
...
```ts
import type { StreakType, Streak } from './streak';
...
export const useUserStore = create<UserState>((set) => ({
  ...
  recordStreak: (type: StreakType) => {
    set(state => {
      ...
      const newStreak: Streak = { current: newCurrent, lastCompletedKey: currentKey };
...
PuzzleScreen currently expects `packId` + `puzzleIndex` and looks up the puzzle from the static `packs` array. For streaks, we need a second entry path.

### Route params change

The Puzzle screen will accept either:
- `{ packId, puzzleIndex }` — existing pack puzzle flow
- `{ streakType }` — streak puzzle flow
...
WinBanner needs to handle both flows. For streaks, show streak count instead of "Next Puzzle".
...
For streak wins:
- Call `useUserStore.getState().recordStreak(streakType)` when completed flips to true
...
const streak = useUserStore(s =>
  streakType ? getActiveStreak(s.streaks[streakType], streakType) : 0
);
...
## Step 7 — HomeScreen Streak Cards

Add three streak preview cards above the pack list.

```tsx
import { streakPacks } from '../packs';
...
| `src/types/streak.ts` | **Create** — StreakType, Streak |
| `src/utils/streakDate.ts` | **Create** — getCurrentKey, getPreviousKey, getPuzzleIndex, getActiveStreak, getISOWeek |
| `src/packs.ts` | **Edit** — add streak pack imports + `streakPacks` export |
...
| `src/screens/HomeScreen.tsx` | **Edit** — add streak preview cards above pack list |
| `src/screens/PuzzleScreen.tsx` | **Edit** — accept streakType param, resolve puzzle from streak pack |
| `src/components/WinBanner.tsx` | **Edit** — accept streakType prop, show streak on win |
...
2. **Step 1**: Create `src/types/streak.ts`
3. **Step 2**: Create `src/utils/streakDate.ts`
4. **Step 3**: Edit `src/packs.ts` (add streak pack imports)
...
8. **Step 7**: Edit `src/screens/HomeScreen.tsx` (streak cards)
...
- [x] **1.1** Create `src/types/streak.ts` with `StreakType` and `Streak`
- [x] **1.2** Create `src/utils/streakDate.ts` with all five functions: `getCurrentKey`, `getPreviousKey`, `getPuzzleIndex`, `getISOWeek`, `getActiveStreak`
...
- [ ] **4.5** Manual test — solve a streak puzzle, verify win banner shows streak and "Back to Home" works
- [ ] **4.6** Manual test — reopen app, verify streak persists and completed streak shows streak count
