...
| Premium (one-time, non-consumable) | `sb_premium_599`  | `payments.ts:34` (`PREMIUM_PRODUCT_ID`) — a lifetime unlock, **not** a subscription |
...
1. **My Apps → Star Battle → In-App Purchases → Non-Consumable** — both
   `sb_premium_599` (the one-time "lifetime" premium unlock) and each
   `starbattle_pack_*`.
...
1. **Monetize → Products → In-app products** → `sb_premium_599` (and each
   `starbattle_pack_*`) — one-time managed products, **not** subscriptions/base plans.
...
- **Console check:** the product shows the per-country price table.
  Confirm LATAM/Europe amounts match intent.
- **Mapping check:** the **product ID** must match what Adapty has mapped (see §3).
...
1. **Apps & Products → Products** → confirm each product is mapped:
   - `sb_premium_599` → App Store product **and** Google in-app product.
   - each `starbattle_pack_*` → both stores.
   - No "unmapped" or error badges.
