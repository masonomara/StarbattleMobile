...
- [x] Add `hintGhosts: Map<number, 'star' | 'mark'>` to `PuzzleState`
- [x] Add `hintStepIndex: number` to `PuzzleState`
- [x] Add `showHint: () => void` to `PuzzleState`
- [x] Add `dismissHint: () => void` to `PuzzleState`

**`src/store.ts` — initial state**

- [x] Add `hintGhosts: new Map()` to store defaults (next to `errorCells`)
- [x] Add `hintStepIndex: -1` to store defaults

**`src/store.ts` — loadPuzzle**

- [x] Reset `hintGhosts: new Map()` in `loadPuzzle`'s `set()` call
- [x] Reset `hintStepIndex: -1` in `loadPuzzle`'s `set()` call

**`src/store.ts` — showHint action**

- [x] Implement `showHint`: early-return if `!puzzle || completed`
- [x] Toggle off: if `hintGhosts.size > 0`, set both to empty/`-1` and return
- [x] Walk `puzzle.hints` from index 0: for each step, build a `Map<number, 'star' | 'mark'>` of unplaced cells
- [x] Use `puzzle.size` (not `boardSize` — store has no `boardSize` field) for index math: `r * puzzle.size + c`
- [x] Compare `cells[idx] !== 1` for placements, `cells[idx] !== 2` for marks
- [x] On first step with non-empty ghosts map, `set({ hintGhosts: ghosts, hintStepIndex: i })` and return

**`src/store.ts` — dismissHint action**

- [x] Implement `dismissHint`: `set({ hintGhosts: new Map(), hintStepIndex: -1 })`
...
- [x] Add guard to `tapCell` (after the `completed` early-return)
- [x] Add guard to `undo` (after the `moveLog.length === 0` early-return)
- [x] Add guard to `redo` (after the `redoStack.length === 0` early-return)
- [x] Add guard to `clearBoard` (after the `completed` early-return)
- [x] Add guard to `applyDrawStroke` (after the `completed` early-return)

### Phase 3: CellView Ghost Rendering ✓

**`src/components/CellView.tsx` — imports**

- [x] Add `View` to the `react-native` import
- [x] ~~Add `STAR_ICON_SIZE` and `MARK_ICON_SIZE` to the `../utils/constants` import~~ (constants don't exist; kept inline `22`/`14`)

**`src/components/CellView.tsx` — selector**

- [x] Add `ghost` to the `useShallow` selector: `ghost: s.hintGhosts.get(idx) ?? null`
- [x] Note: current selector uses `row * s.puzzle!.size + col` — keep that pattern (no `boardSize`)

**`src/components/CellView.tsx` — JSX**

- [x] Render ghost star overlay: `ghost === 'star' && value !== 1` → `<View style={styles.ghost}><StarIcon ... color={theme.accent} /></View>`
- [x] Render ghost mark overlay: `ghost === 'mark' && value !== 2` → `<View style={styles.ghost}><MarkIcon ... color={theme.accent} /></View>`
- [x] Place ghost elements after the real value elements (ghosts sit on top visually but opacity makes them recessive)
