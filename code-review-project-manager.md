...
Then the render adds a _second_ time display for streaks:
...
The `user_entitlements` watch fires → `loadEntitlements(userId)` runs the same `PACK_QUERY` internally _and_ sets `packCatalog` again.
...
_The water does not try to be water. It simply finds the lowest place and rests there. The code that needs the most fixing is the code that is trying hardest to be something it isn't._
